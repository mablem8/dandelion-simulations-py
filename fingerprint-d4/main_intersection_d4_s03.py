...
import csv
with open('data-intersection-d4-s03.csv', 'w', newline='') as csvfile:
...
for p in ps:
  ...
  for trial in range(num_trials):
    ...
    adobs = generate_intersection_observations(graph, 3)
  ...
  with open('data-intersection-d4-s03.csv', 'a', newline='') as csvfile:
    datawriter = csv.writer(csvfile, delimiter=',')
